...
class ConditionalDETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, aux_loss=False):
        ...
        num_backbone_outs = len(backbone.out_indices)
        input_proj_list = []
        for _ in range(num_backbone_outs):
            in_channels = backbone.num_features[_]
        ...
        self.position_embedding = PositionEmbeddingSine(128, normalize=True) # args.hidden_dim // 2
    ...
    def forward(self, samples: NestedTensor):
        ...
        for name, src in enumerate(srcs):
            shapes.append(src.shape[-2:])
